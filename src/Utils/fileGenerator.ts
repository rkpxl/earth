...
const supportedLanguages = [{ languageCode: 'hi', fontPath: '/static/fonts/hindi.ttf' }]
    ...
    Object.keys(formState?.tabs)?.forEach((tabIndex) => {
      ...
      if (tab?.questions) {
        Object.keys(tab?.questions)?.forEach((questionId) => {
          ...
          const font =
            supportedLanguages.find((language) => language.languageCode === detectedLanguageCode)
              ?.languageCode || 'times'
          pdf.setFont(font)
          ...
          if (question?.questionTitle) {
            ...
            const fontanswer =
              supportedLanguages.find(
                (language) => language.languageCode === detectedLanguageCodeabswer,
              )?.languageCode || 'times'
