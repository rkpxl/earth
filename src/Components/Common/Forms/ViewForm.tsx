...
export const ViewForm: React.FC = () => {
  ...
  const submitHandle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (reviewer) {
      ...
      const updatedJson = await axios.post(
        process.env.NEXT_PUBLIC_HOST_URL + '/tasks/' + taskId + '/update-raw-json',
        {
      ...
      axios
        .post(process.env.NEXT_PUBLIC_HOST_URL + '/tasks/review', {
  ...
  React.useEffect(() => {
    ...
    if (id) {
      axios
        .get(`${process.env.NEXT_PUBLIC_HOST_URL}/tasks/${id}`)
        .then((response) => {
          setResult(JSON.parse(response?.data?.rawJson))
          const filteredPeoples = response?.data?.approvals?.filter(
            (p: any) => !(p.status === 'APPROVED' || p.userId === localStorage.getItem('_id')),
          )
          const selected = response?.data?.approvals?.filter(
            (p: any) => p.userId === localStorage.getItem('_id'),
          )
