...
export default function FormPersonnel({ compliance, protocol }: IProps) {
  ...
  const updateApprovers = async (approver : IApprover, approvers : IApprover[], isDelete : boolean) => {
    if((approver.isNew && approver.role && approver.access) || isDelete) {
      try {
        const result = await axiosInstance.put('/protocol', {
          _id: protocol._id,
          approvers: approvers.filter((ap) => ap.access && ap.name && ap.role && ap.user_id).map(({ isNew, ...rest }, index) => {
            if(isNew) {
              return {
                ...rest,
                step: index,
        ...
        <Paper key={index} elevation={3} style={{ padding: '10px', margin: '10px' }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={1} sx={{ display: 'flex', justifyContent: 'center' }}>
              <IconButton 
                onClick={() => removeRow(index)} color="secondary" aria-label="delete"
              >
