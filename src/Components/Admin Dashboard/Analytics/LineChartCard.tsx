...
const processData = (data: StatusCount[]) => {
  const sortedDays = Array.from(new Set(data.map(item => {
  ...
  const datasets: Record<string, { label: string; borderColor: string; data: (number | null)[] }> = {
    "Approved": { label: "Approved", borderColor: getAnalyticsColor(0), data: new Array(sortedDays.length).fill(null) },
    "Pending": { label: "Pending", borderColor: getAnalyticsColor(1), data: new Array(sortedDays.length).fill(null) },
    "Draft": { label: "Draft", borderColor: getAnalyticsColor(2), data: new Array(sortedDays.length).fill(null) },
    "Other": { label: "Multiple States", borderColor: getAnalyticsColor(3), data: new Array(sortedDays.length).fill(null) },
  };
