...
export default function AddGroupMember(props:any) {
  ...
  const { data: peoples, isLoading: peopleIsLoading, isError : peopleIsError } = useQuery({
  ...
  const { data: members, isLoading: memberIsLoading, isError : memberIdError } = useQuery({
            ...
            <Card key={ppl.email} sx={{ mt: 1.5, borderRadius: '16px', display: "flex", alignItems: "center"}}>
              <Grid>
                <Checkbox
                  ...
                  onChange={(e) => { handleToggle(ppl._id) }} 
